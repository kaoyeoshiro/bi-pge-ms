...
@router.post("/enable-unaccent")
async def enable_unaccent_extension(
    session: AsyncSession = Depends(get_session),
    _: bool = Depends(verify_admin),
):
    """
    [TEMPORÁRIO] Habilita a extensão unaccent no PostgreSQL.
    Necessário para a busca textual de assuntos sem considerar acentos.
    """
    ...
    try:
        ...
        if exists:
            return {
                ...
                "message": "Extensão unaccent já está habilitada"
            }

        # Habilitar a extensão
        await session.execute(text("CREATE EXTENSION unaccent"))
        await session.commit()

        # Teste rápido
        test_result = await session.execute(
            text("SELECT unaccent('José') as teste")
        )
        test_value = test_result.scalar()

        return {
            "status": "enabled",
            "message": "Extensão unaccent habilitada com sucesso",
            "test": f"unaccent('José') = '{test_value}'"
        }

    except Exception as e:
        logger.exception("Erro ao habilitar extensão unaccent")
        raise HTTPException(
            status_code=500,
            detail=f"Erro ao habilitar extensão: {str(e)}"
        )
